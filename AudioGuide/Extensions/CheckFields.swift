...
//  AudioGuide
//
//  Created by Максим Оліярник on 02.11.2023.
//
import UIKit
    ...
    func validField(_ parentView: UIView, _ field: UITextField) -> Bool{
        ...
        case "email":
            if isValid("e", field.text!) {
               validView(parentView, field, true)
                print("email succesfull veryfied")
                return true
            } else {
                validView(parentView, field, false)
                return false
        ...
        case "emailForReset":
            if isValid("eR", field.text!) {
               validView(parentView, field, true)
                print("reset email succesfull veryfied")
                return true
            } else {
                validView(parentView, field, false)
                return false
        ...
        case "password":
            if isValid("p", field.text!) {
                validView(parentView, field, true)
                print("password succesfull veryfied")
                return true
            } else {
                validView(parentView, field, false)
                return false
        ...
        case "confirmPassword":
            if isValid("cp", field.text!) {
                validView(parentView, field, true)
                print("confirm succesfull veryfied")
                return true
            } else {
                validView(parentView, field, false)
                return false
        ...
        case "phoneNumber":
            if field.text?.first != "+" || field.text?.count ?? 0 < 13{
                validView(parentView, field, false)
                return false
            }else {
                validView(parentView, field, true)
